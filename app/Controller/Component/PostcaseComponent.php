...
class PostcaseComponent extends CookieComponent {
    ...
    function casePosting($formdata) {
        ...
        $cloud_storages = $formdata['cloud_storages']; //By S2LSolutions
...
####### Case Format
        if (isset($cloud_storages) && !empty($cloud_storages)) { //By S2LSolutions
...
<span style='font-size:18px;'>S2LSolutions</span> - Daily Catch-Up Alert
</td>
</tr>
<tr>
...
This is a reminder to post your today's updates to S2LSolutions. Just reply to this email with the updates, it will be added to the project.
...
//			    
                            if (@$findEmail['User']['id']) {
                                $subject = $fromName . " invited you to join " . CMP_SITE . " on S2LSolutions";
                                $ext_user = 1;
                            } else {
