<?php
/*********************************************************************************
 * S2LSolutions Community Edition is a web based Project Management software developed by
 * S2LSolutions. Copyright (C) 2013-2014
 *
 ...
 * 02110-1301 USA.
 *
 * You can contact S2LSolutions, 2059 Camden Ave. #118, San Jose, CA - 95124, US.
   or at email address info@s2lsolutions.com.
 *
 ...
 * S2LSolutions" logo. If the display of the logo is not reasonably feasible for
 * technical reasons, the Appropriate Legal Notices must display the words
 * "Powered by S2LSolutions".
 ********************************************************************************/
/*********************************************************************************
 * Description:  Defines the Cron jobs for all CLI actions
 * Portions created by S2LSolutions are Copyright (C) S2LSolutions.
 * All Rights Reserved.
 ********************************************************************************/
...
//echo "<pre>";print_r($userInfo);//exit;
				...
				if($usr['UserNotification']['value'] == 1) {
					...
					if($sub) {
						$subject = $sub." Tasks on S2LSolutions - ".date("m/d",strtotime(GMT_DATE));
					} else {
						$subject = 'S2LSolutions Daily Task Status - '.date("m/d",strtotime(GMT_DATE));
					}
				}
				elseif($usr['UserNotification']['value'] == 2) {
					$sb_title = 'Weekly Task Status Updates';
					if($sub) {
						$subject = $sub." on S2LSolutions - ".date("m/d",strtotime($upto))." - ".date("m/d",strtotime(GMT_DATE));
					} else {
						$subject = 'S2LSolutions Weekly Task Status - '.date("m/d",strtotime($upto))." - ".date("m/d",strtotime(GMT_DATE));
					}
				}
				elseif($usr['UserNotification']['value'] == 3) {
					...
					if($sub) {
						...
						$subject = 'S2LSolutions Monthly Task Status - '.date("m/d",strtotime($upto))." - ".date("m/d",strtotime(GMT_DATE));
					}
				}
				...
				$message = "<table cellpadding='0' cellspacing='0' align='left' style='width:100%'>
							...
							<tr>
								<td align='left' style='font:10px Arial;padding-top:2px;color:#737373'>
									You are receiving this email notification because you have subscribed to S2LSolutions Task Status E-mail notification, to unsubscribe, please click <a href='".HTTP_ROOT."users/email_notifications' target='_blank'>Unsubscribe Email Notification</a>
								</td>	  
							</tr>
						</table>";
	...
	function update_email(){
		...
		foreach($getAllNot as $emlNot) { 
			...
			if(count($case_cming) || count($case_late)){
				...
				//$subject = "S2LSolutions Task Due notification: ".$this->Format->dateFormatReverse($tmzone->GetDateTime($usr['User']['timezone_id'],$timezn['Timezone']['gmt_offset'],$timezn['Timezone']['dst_offset'],$timezn['Timezone']['code'],$gmt_datetime,"date"));
				
				$subject = $sub." on S2LSolutions - ".date("m/d",strtotime($taskduedt));
				$message = "<table cellpadding='0' cellspacing='0' align='left' width='100%'>
								...
								<tr>
									<td align='left' style='font:10px Arial;padding-top:2px;color:#737373'>
									You are receiving this email notification because you have subscribed to S2LSolutions, to unsubscribe, please click <a href='".HTTP_ROOT."users/email_notifications' target='_blank'>Unsubscribe Email Notification</a>
								</td>
								</tr>
							</table>";
						...
						$message = "<tr><td><table style='border-collapse:collapse;border-spacing:0;text-align:left;width:600px;border:1px solid #5191BD'>
								<tr style='background:#5191BD;height:50px;'>
									<td style='font:bold 14px Arial;padding:10px;color:#FFFFFF;'>
										<span style='font-size:18px;'>S2LSolutions</span> - Daily Task Updates
									</td>
								</tr>
								<tr>
 ...
 */
	function weeklyusagedetails(){//echo "<pre>";
		...
		foreach($user_details AS $key=>$val){
			...
			$subject = "S2LSolutions Usage Report ".$frmdt." - ".$todt;
		...
		<div><ul><li><b>'.$userlogin[0][0]['notlogged'].'</b> Out of <b>'.$userlogin[0][0]['tot'].'</b> User has not logged in to the system since last week.
				...
				$message .="</div></div>";
				$message .="<div style='clear:both'></div><div style='font-size:11px;padding-top:10px;color:#737373'>Don't want to receive this email?<br/>Go to the S2LSolutions <a href='".HTTP_ROOT."users/email_notifications'>notification settings</a> and say NO to <b>Weekly Usage Report</b></div>";
