...
<!--		<div class="fl" style="position: relative;">&nbsp;&nbsp;
	...
	<?php if((SES_TYPE == 1 || SES_TYPE == 2) && defined('TWITTED') && TWITTED == 0 && !in_array(PAGE_NAME,array('onbording','profile','changepassword','email_notifications','email_reports'))){
		$osTwAccount = 'TheS2LSolutions';
		$tweetTexts = array(
			'#S2LSolutions is an Awesome Project Collaboration Tool that gives full visibility and control over your projects',
			'Organize Projects, Tasks, Documents & Meeting Minutes in one place #S2LSolutions'.' @'.$osTwAccount,
			'Share of your ideas, feedbacks, questions and discussions across the team #S2LSolutions'.' @'.$osTwAccount,
			'See what\'s in progress, what needs to be done and what\'s been accomplished #S2LSolutions'.' @'.$osTwAccount,
			'Receive reminders, alert for close deadlines, manage tightly not to exceed budget #S2LSolutions'.' @'.$osTwAccount,
			'Break-down tasks into smaller ones, share documents using google Drive & Dropbox #S2LSolutions'.' @'.$osTwAccount,
			'Just sit back and keep on watching the Activity even while relaxing #S2LSolutions'.' @'.$osTwAccount,
			'Win your customers\'confidence by keeping them informed with daily scrum #S2LSolutions'.' @'.$osTwAccount,
			'Keep your team on their toes by reminding them by automatic emails #S2LSolutions'.' @'.$osTwAccount,
			'Get instant notification on your cell and respond with your inputs in no time #S2LSolutions'.' @'.$osTwAccount,
			'Stay on top and get weekly usage report #S2LSolutions'.' @'.$osTwAccount,
			'Managing Project Effectively with project collaboration tool taming inbox #S2LSolutions'.' @'.$osTwAccount,
			'Get Daily Progress email from team without fail #S2LSolutions'.' @'.$osTwAccount,
			'#S2LSolutions is a Awesome project management tool for You & Your Team'.' @'.$osTwAccount
		);
