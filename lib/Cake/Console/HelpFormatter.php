 ...
 */
	public function text($width = 72) {
		...
		if (!empty($description)) {
			$out[] = Str::wrap($description, $width);
			$out[] = '';
		}
		...
		if (!empty($subcommands)) {
			...
			foreach ($subcommands as $command) {
				$out[] = Str::wrap($command->help($max), array(
					'width' => $width,
		...
		if (!empty($options)) {
			...
			foreach ($options as $option) {
				$out[] = Str::wrap($option->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
		...
		if (!empty($arguments)) {
			...
			foreach ($arguments as $argument) {
				$out[] = Str::wrap($argument->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
