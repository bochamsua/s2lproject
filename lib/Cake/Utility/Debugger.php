	...
	public static function trace($options = array()) {
		...
		for ($i = $options['start']; $i < $count && $i < $options['depth']; $i++) {
			...
			if ($options['format'] === 'points' && $trace['file'] !== '[internal]') {
				...
				$back[] = Str::insert($tpl, $trace, array('before' => '{:', 'after' => '}'));
			}
		}
 ...
 *
 * Where $data is an array of strings that use Str::insert() variable
	...
	public function outputError($data) {
		...
		if (isset($tpl['links'])) {
			foreach ($tpl['links'] as $key => $val) {
				$links[$key] = Str::insert($val, $data, $insertOpts);
			}
		}
		...
		foreach ($infoData as $key => $value) {
			...
			$info .= Str::insert($tpl[$key], array($key => $value) + $data, $insertOpts);
		}
		$links = implode(' ', $links);
